        ...
        with conn:
            ...
            while True:
                ...
                if data.decode().startswith("INCREASE PERIOD:"):
                    ...
                    settings[period_id][1] -= time_to_add
                    conn.sendall("CONNECTION OK".encode())  # Send OK after processing
                elif data.decode().startswith("REMOVE PERIOD:"):
                    period_id = int(data.decode()[15:])
                    settings.pop(period_id)
                    for i in range(period_id, len(settings)): # Fix the numbers
                        settings[i][0] = f"period_{i+1}"
                    conn.sendall("CONNECTION OK".encode())  # Send OK after processing
                elif data.decode() == "CREATE PERIOD":
